...
class InteractionManager {
  ...
  handleMouseOut(e) {
    ...
    if (element && !element.classList.contains("selected")) {
      element.classList.remove("hover");  // ← Supprime juste hover
    }
  }
  ...
  handleWheel(e) {
    ...
    this.onCodeGenerate?.(element, false);
  }
...
class CodeGenerator {
  ...
  async loadTemplates() {
    try {
      const response = await fetch('./csspageweaver/plugins/layout/shortcode-templates.json');
  ...
  generateMarkdownCode(element) {
    ...
    return template ? this.applyTemplate(template, element, 'markdown') : '';
  }

  generateImageCode(element) {
    const template = this.templates.formats[this.templates.defaultFormat].image;
...
export default class Layout extends Handler {
  ...
  getCSSProperties(element) {
    const propertyMap = {
      ...
      "printrow": "--print-row",
      "printheight": "--print-height",
      "alignself": "--align-self",  // ← Mapping explicit
      "figcaption_arrow": "--figcaption_arrow",
      "img-x": "--img-x",
      "img-y": "--img-y",
      "img-w": "--img-w"
    };
    ...
    const result = {};
    Object.entries(propertyMap).forEach(([key, cssVar]) => {
      result[key] = element.style.getPropertyValue(cssVar) || "";
    });
